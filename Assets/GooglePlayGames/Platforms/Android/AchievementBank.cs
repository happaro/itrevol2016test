...
namespace GooglePlayGames.Android {
    internal class AchievementBank {
        ...
        internal void ProcessBuffer(AndroidJavaObject achBuffer) {
            ...
            GooglePlayGames.OurUtils.Logger.d("AchievementBank: processing achievement buffer given as Java object.");

            if (achBuffer == null) {
                GooglePlayGames.OurUtils.Logger.w("AchievementBank: given buffer was null. Ignoring.");
                return;
            }

            count = achBuffer.Call<int>("getCount");
            GooglePlayGames.OurUtils.Logger.d("AchievementBank: buffer contains " + count + " achievements.");

            for (i = 0; i < count; ++i) {
                GooglePlayGames.OurUtils.Logger.d("AchievementBank: processing achievement #" + i);
                Achievement ach = new Achievement();
                AndroidJavaObject achObj = achBuffer.Call<AndroidJavaObject>("get", i);

                if (achObj == null) {
                    GooglePlayGames.OurUtils.Logger.w("Achievement #" + i + " was null. Ignoring.");
                    continue;
                }
                ...
                GooglePlayGames.OurUtils.Logger.d("AchievementBank: processed: " + ach.ToString());
                if (ach.Id != null && ach.Id.Length > 0) {
                    mAchievements[ach.Id] = ach;
                } else {
                    GooglePlayGames.OurUtils.Logger.w("Achievement w/ missing ID received. Ignoring.");
                }
            }

            GooglePlayGames.OurUtils.Logger.d("AchievementBank: bank now contains " + mAchievements.Count + " entries.");
        }
        ...
        internal Achievement GetAchievement(string id) {
            if (mAchievements.ContainsKey(id)) {
                ...
                GooglePlayGames.OurUtils.Logger.w("Achievement ID not found in bank: id " + id);
                return null;
            }
        }
