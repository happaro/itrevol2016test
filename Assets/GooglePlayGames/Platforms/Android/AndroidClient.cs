...
namespace GooglePlayGames.Android {
    public class AndroidClient : IPlayGamesClient {
        ...
        public AndroidClient() {
            ...
            RunOnUiThread(() => {
                GooglePlayGames.OurUtils.Logger.d("Initializing Android Client.");
                GooglePlayGames.OurUtils.Logger.d("Creating GameHelperManager to manage GameHelper.");
        ...
        public void Authenticate(System.Action<bool> callback, bool silent) {
            if (mAuthState != AuthState.NoAuth) {
                GooglePlayGames.OurUtils.Logger.w("Authenticate() called while an authentication process was active. " + 
                        mAuthState);
            ...
            GooglePlayGames.OurUtils.Logger.d("Making sure PlayGamesHelperObject is ready.");
            PlayGamesHelperObject.CreateObject();
            GooglePlayGames.OurUtils.Logger.d("PlayGamesHelperObject created.");

            mSilentAuth = silent;
            GooglePlayGames.OurUtils.Logger.d("AUTH: starting auth process, silent=" + mSilentAuth);
            RunOnUiThread(() => {
                switch (mGHManager.State) {
                    case GameHelperManager.ConnectionState.Connected:
                        GooglePlayGames.OurUtils.Logger.d("AUTH: already connected! Proceeding to achievement load phase.");
                        mAuthCallback = callback;
                        DoInitialAchievementLoad();
                        break;
                    case GameHelperManager.ConnectionState.Connecting:
                        GooglePlayGames.OurUtils.Logger.d("AUTH: connection in progress; auth now pending.");
                        mAuthCallback = callback;
                    ...
                    default:
                        mAuthCallback = callback;
                        if (mSilentAuth) {
                            GooglePlayGames.OurUtils.Logger.d("AUTH: not connected and silent=true, so failing.");
                            mAuthState = AuthState.NoAuth;
                            InvokeAuthCallback(false);
                        } else {
                            GooglePlayGames.OurUtils.Logger.d("AUTH: not connected and silent=false, so starting flow.");
        ...
        private void DoInitialAchievementLoad() {
            GooglePlayGames.OurUtils.Logger.d("AUTH: Now performing initial achievement load...");
            ...
            GooglePlayGames.OurUtils.Logger.d("AUTH: Initial achievement load call made.");
        }
        
        // UI thread
        private void OnAchievementsLoaded(int statusCode, AndroidJavaObject buffer) {
            if (mAuthState == AuthState.LoadingAchs) {
                GooglePlayGames.OurUtils.Logger.d("AUTH: Initial achievement load finished.");
                ...
                if (statusCode == JavaConsts.STATUS_OK || 
                    ...
                    GooglePlayGames.OurUtils.Logger.d("Processing achievement buffer.");
                    mAchievementBank.ProcessBuffer(buffer);
                    
                    GooglePlayGames.OurUtils.Logger.d("Closing achievement buffer.");
                    buffer.Call("close");
                    
                    GooglePlayGames.OurUtils.Logger.d("AUTH: Auth process complete!");
                    ...
                    GooglePlayGames.OurUtils.Logger.w("AUTH: Failed to load achievements, status code " + statusCode);
                    mAuthState = AuthState.NoAuth;
                    InvokeAuthCallback(false);
                }
            } else {
                GooglePlayGames.OurUtils.Logger.w("OnAchievementsLoaded called unexpectedly in auth state " + mAuthState);
            }
        }
        ...
        private void InvokeAuthCallback(bool success) {
            ...
            GooglePlayGames.OurUtils.Logger.d("AUTH: Calling auth callback: success=" + success);
        ...
        private void RetrieveUserInfo() {
            GooglePlayGames.OurUtils.Logger.d("Attempting to retrieve player info.");
            ...
            using (AndroidJavaObject playerObj = mGHManager.CallGmsApi<AndroidJavaObject>(
                ...
                if (mUserId == null) {
                    ...
                    GooglePlayGames.OurUtils.Logger.d("Player ID: " + mUserId);
                }
    
                if (mUserDisplayName == null) {                
                    mUserDisplayName = playerObj.Call<string>("getDisplayName");
                    GooglePlayGames.OurUtils.Logger.d("Player display name: " + mUserDisplayName);
                }
            }
        }
        ...
        internal void OnSignInSucceeded() {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient got OnSignInSucceeded.");
            RetrieveUserInfo();

            if (mAuthState == AuthState.AuthPending || mAuthState == AuthState.InProgress) {
                GooglePlayGames.OurUtils.Logger.d("AUTH: Auth succeeded. Proceeding to achievement loading.");
                DoInitialAchievementLoad();
            } else if (mAuthState == AuthState.LoadingAchs) {
                GooglePlayGames.OurUtils.Logger.w("AUTH: Got OnSignInSucceeded() while in achievement loading phase (unexpected).");
                GooglePlayGames.OurUtils.Logger.w("AUTH: Trying to fix by issuing a new achievement load call.");
                DoInitialAchievementLoad();
            } else {
                ...
                GooglePlayGames.OurUtils.Logger.d("Normal lifecycle OnSignInSucceeded received.");
                RunPendingActions();
        ...
        internal void OnSignInFailed() {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient got OnSignInFailed.");
            if (mAuthState == AuthState.AuthPending) {
                // we have yet to start the auth flow
                if (mSilentAuth) {
                    GooglePlayGames.OurUtils.Logger.d("AUTH: Auth flow was pending, but silent=true, so failing.");
                    mAuthState = AuthState.NoAuth;
                    InvokeAuthCallback(false);
                } else {
                    GooglePlayGames.OurUtils.Logger.d("AUTH: Auth flow was pending and silent=false, so doing noisy auth.");
                    mAuthState = AuthState.InProgress;
                    mGHManager.BeginUserInitiatedSignIn();
                }
            } else if (mAuthState == AuthState.InProgress) {
                // authentication was in progress, but failed: notify callback
                GooglePlayGames.OurUtils.Logger.d("AUTH: FAILED!");
                mAuthState = AuthState.NoAuth;
                InvokeAuthCallback(false);
            } else if (mAuthState == AuthState.LoadingAchs) {
                // we were loading achievements and got disconnected: notify callback
                GooglePlayGames.OurUtils.Logger.d("AUTH: FAILED (while loading achievements).");
                mAuthState = AuthState.NoAuth;
                InvokeAuthCallback(false);
            } else if (mAuthState == AuthState.NoAuth) {
                // we will hit this case during the normal lifecycle (for example, Activity
                // was brought to the foreground and sign in has failed).
                GooglePlayGames.OurUtils.Logger.d("Normal OnSignInFailed received.");
            } else if (mAuthState == AuthState.Done) {
                // we lost authentication (for example, the token might have expired,
                // or the user revoked it)
                GooglePlayGames.OurUtils.Logger.e("Authentication has been lost!");
                mAuthState = AuthState.NoAuth;
            }
        }
        ...
        private void RunPendingActions() {
            if (mActionsPendingSignIn.Count > 0) {
                GooglePlayGames.OurUtils.Logger.d("Running pending actions on the UI thread.");
                while (mActionsPendingSignIn.Count > 0) {
                    Action a = mActionsPendingSignIn[0];
                    mActionsPendingSignIn.RemoveAt(0);
                    a.Invoke();
                }
                GooglePlayGames.OurUtils.Logger.d("Done running pending actions on the UI thread.");
            } else {
                GooglePlayGames.OurUtils.Logger.d("No pending actions to run on UI thread.");
            }
        }
        ...
        public void SignOut() {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.SignOut");
            mSignOutInProgress = true;
            RunWhenConnectionStable(() => {
                GooglePlayGames.OurUtils.Logger.d("Calling GHM.SignOut");
                mGHManager.SignOut();
                mAuthState = AuthState.NoAuth;
                mUserId = null;
                mUserDisplayName = null;
                mSignOutInProgress = false;
                GooglePlayGames.OurUtils.Logger.d("Now signed out.");
            });
        }
        ...
        private class OnAchievementsLoadedResultProxy : AndroidJavaProxy {
            ...
            public void onResult(AndroidJavaObject result) {
                GooglePlayGames.OurUtils.Logger.d("OnAchievementsLoadedResultProxy invoked");
                GooglePlayGames.OurUtils.Logger.d("    result=" + result);
        ...
        private void RunWhenConnectionStable(Action a) {
            RunOnUiThread(() => {
                if (mGHManager.Paused || mGHManager.Connecting) {
                    ...
                    GooglePlayGames.OurUtils.Logger.d("Action scheduled for later (connection currently in progress).");
        ...
        internal void CallClientApi(string desc, Action call, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("Requesting API call: " + desc);
            RunWhenConnectionStable(() => {
                ...
                if (mGHManager.IsConnected()) {
                    ...
                    GooglePlayGames.OurUtils.Logger.d("Connected! Calling API: " + desc);
                    call.Invoke();
                    if (callback != null) {
                        PlayGamesHelperObject.RunOnGameThread(() => {
                    ...
                    GooglePlayGames.OurUtils.Logger.w("Not connected! Failed to call API :" + desc);
                    if (callback != null) {
                        PlayGamesHelperObject.RunOnGameThread(() => {
        ...
        public void UnlockAchievement(string achId, Action<bool> callback) {
            ...
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.UnlockAchievement: " + achId);
            Achievement a = GetAchievement(achId);
            if (a != null && a.IsUnlocked) {
                GooglePlayGames.OurUtils.Logger.d("...was already unlocked, so no-op.");
                if (callback != null) {
                    callback.Invoke(true);
                }
                return;
        ...
        public void RevealAchievement(string achId, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.RevealAchievement: " + achId);
            Achievement a = GetAchievement(achId);
            if (a != null && a.IsRevealed) {
                GooglePlayGames.OurUtils.Logger.d("...was already revealed, so no-op.");
                if (callback != null) {
                    callback.Invoke(true);
                }
                return;
        ...
        public void IncrementAchievement(string achId, int steps, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.IncrementAchievement: " + achId + ", steps " + steps);
        ...
        public void ShowAchievementsUI() {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.ShowAchievementsUI.");
            CallClientApi("show achievements ui", () => {
                using (AndroidJavaObject intent = mGHManager.CallGmsApi<AndroidJavaObject>(
                    ...
                    using (AndroidJavaObject activity = GetActivity()) {
                        GooglePlayGames.OurUtils.Logger.d("About to show achievements UI with intent " + intent +
                            ", activity " + activity);
        ...
        public void ShowLeaderboardUI(string lbId) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.ShowLeaderboardUI, lb=" + (lbId == null ? "(all)" : lbId));
            CallClientApi("show LB ui", () => {
                using (AndroidJavaObject intent = GetLeaderboardIntent(lbId)) {
                    using (AndroidJavaObject activity = GetActivity()) {
                        GooglePlayGames.OurUtils.Logger.d("About to show LB UI with intent " + intent +
                            ", activity " + activity);
                        if (intent != null && activity != null) {
                            activity.Call("startActivityForResult", intent, RC_UNUSED);
                        }
        ...
        public void SubmitScore(string lbId, long score, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.SubmitScore, lb=" + lbId + ", score=" + score);
        ...
        public void LoadState(int slot, OnStateLoadedListener listener) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.LoadState, slot=" + slot);
        ...
        public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
            GooglePlayGames.OurUtils.Logger.d(string.Format("AndroidClient.UpdateState, slot={0}, data={1}",
                slot, GooglePlayGames.OurUtils.Logger.describe(data)));
        ...
        public void SetCloudCacheEncrypter(BufferEncrypter encrypter) {
            GooglePlayGames.OurUtils.Logger.d("Ignoring cloud cache encrypter (not used in Android)");
        ...
        public void RegisterInvitationDelegate(InvitationReceivedDelegate deleg) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.RegisterInvitationDelegate");
            if (deleg == null) {
                GooglePlayGames.OurUtils.Logger.w("AndroidClient.RegisterInvitationDelegate called w/ null argument.");
                return;
            }
            ...
            if (!mRegisteredInvitationListener) {
                GooglePlayGames.OurUtils.Logger.d("Registering an invitation listener.");
                RegisterInvitationListener();
            }
            
            if (mInvitationFromNotification != null) {
                GooglePlayGames.OurUtils.Logger.d("Delivering pending invitation from notification now.");
        ...
        public Invitation GetInvitationFromNotification() {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.GetInvitationFromNotification");
            GooglePlayGames.OurUtils.Logger.d("Returning invitation: " + ((mInvitationFromNotification == null) ?
        ...
        public bool HasInvitationFromNotification() {
            ...
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.HasInvitationFromNotification, returning " + has);
            return has;
        }
        
        
        private void RegisterInvitationListener() {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.RegisterInvitationListener");
        ...
        internal void ClearInvitationIfFromNotification(string invitationId) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.ClearInvitationIfFromNotification: " + invitationId);
            if (mInvitationFromNotification != null && 
                mInvitationFromNotification.InvitationId.Equals(invitationId)) {
                GooglePlayGames.OurUtils.Logger.d("Clearing invitation from notification: " + invitationId);
                mInvitationFromNotification = null;
            }
        }
        ...
        private void CheckForConnectionExtras() {
            ...
            GooglePlayGames.OurUtils.Logger.d("AndroidClient: CheckInvitationFromNotification.");
            GooglePlayGames.OurUtils.Logger.d("AndroidClient: looking for invitation in our GameHelper.");
            ...
            if (invObj != null) {
                GooglePlayGames.OurUtils.Logger.d("Found invitation in GameHelper. Converting.");
                invFromNotif = ConvertInvitation(invObj);
                GooglePlayGames.OurUtils.Logger.d("Found invitation in our GameHelper: " + invFromNotif);
            } else {
                GooglePlayGames.OurUtils.Logger.d("No invitation in our GameHelper. Trying SignInHelperManager.");
                ...
                using (AndroidJavaObject inst = cls.CallStatic<AndroidJavaObject>("getInstance")) {
                    if (inst.Call<bool>("hasInvitation")) {
                        ...
                        GooglePlayGames.OurUtils.Logger.d("Found invitation in SignInHelperManager: " + invFromNotif);
                        inst.Call("forgetInvitation");
                    } else {
                        GooglePlayGames.OurUtils.Logger.d("No invitation in SignInHelperManager either.");
                    }
                }
            }
            ...
            if (matchObj != null) {
                GooglePlayGames.OurUtils.Logger.d("Found match in GameHelper. Converting.");
                match = JavaUtil.ConvertMatch(mUserId, matchObj);
                GooglePlayGames.OurUtils.Logger.d("Match from GameHelper: " + match);
            } else {
                GooglePlayGames.OurUtils.Logger.d("No match in our GameHelper. Trying SignInHelperManager.");
                AndroidJavaClass cls = JavaUtil.GetClass(JavaConsts.SignInHelperManagerClass);
                using (AndroidJavaObject inst = cls.CallStatic<AndroidJavaObject>("getInstance")) {
                    if (inst.Call<bool>("hasTurnBasedMatch")) {
                        match = JavaUtil.ConvertMatch(mUserId, 
                                inst.Call<AndroidJavaObject>("getTurnBasedMatch"));
                        GooglePlayGames.OurUtils.Logger.d("Found match in SignInHelperManager: " + match);
                        inst.Call("forgetTurnBasedMatch");
                    } else {
                        GooglePlayGames.OurUtils.Logger.d("No match in SignInHelperManager either.");
                    }
                }
            }
            ...
            if (invFromNotif != null) {
                if (mInvitationDelegate != null) {
                    GooglePlayGames.OurUtils.Logger.d("Invoking invitation received delegate to deal with invitation " + 
                             " from notification.");
                    PlayGamesHelperObject.RunOnGameThread(() => {
                        if (mInvitationDelegate != null) {
                    ...
                    GooglePlayGames.OurUtils.Logger.d("No delegate to handle invitation from notification; queueing.");
        ...
        private Invitation ConvertInvitation(AndroidJavaObject invObj) {
            GooglePlayGames.OurUtils.Logger.d("Converting Android invitation to our Invitation object.");
            ...
            case JavaConsts.INVITATION_TYPE_TURN_BASED:
                ...
                break;
            default:
                GooglePlayGames.OurUtils.Logger.e("Unknown invitation type " + invType);
                type = Invitation.InvType.Unknown;
                break;
            }
            
            Invitation result = new Invitation(type, invitationId, inviter, variant);
            GooglePlayGames.OurUtils.Logger.d("Converted invitation: " + result.ToString());
            return result;
        }
        
        private void OnInvitationReceived(AndroidJavaObject invitationObj) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.OnInvitationReceived. Converting invitation...");
            Invitation inv = ConvertInvitation(invitationObj);
            GooglePlayGames.OurUtils.Logger.d("Invitation: " + inv.ToString());
            
            if (mInvitationDelegate != null) {
                GooglePlayGames.OurUtils.Logger.d("Delivering invitation to invitation received delegate.");
                PlayGamesHelperObject.RunOnGameThread(() => {
                    if (mInvitationDelegate != null) {
                ...
                GooglePlayGames.OurUtils.Logger.w("AndroidClient.OnInvitationReceived discarding invitation because " + 
                        " delegate is null.");
            }
        }
        
        private void OnInvitationRemoved(string invitationId) {
            GooglePlayGames.OurUtils.Logger.d("AndroidClient.OnInvitationRemoved: " + invitationId);
        ...
        private class OnInvitationReceivedProxy : AndroidJavaProxy {
            ...
            public void onInvitationReceived(AndroidJavaObject invitationObj) {
                GooglePlayGames.OurUtils.Logger.d("OnInvitationReceivedProxy.onInvitationReceived");
                mOwner.OnInvitationReceived(invitationObj);
            }
            
            public void onInvitationRemoved(string invitationId) {
                GooglePlayGames.OurUtils.Logger.d("OnInvitationReceivedProxy.onInvitationRemoved");
