...
namespace GooglePlayGames.Android {
    internal class AndroidTbmpClient : ITurnBasedMultiplayerClient {
        ...
        public void OnSignInSucceeded() {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.OnSignInSucceeded");
            GooglePlayGames.OurUtils.Logger.d("Querying for max match data size...");
            mMaxMatchDataSize = mClient.GHManager.CallGmsApi<int>("games.Games",
                    "TurnBasedMultiplayer", "getMaxMatchDataSize");
            GooglePlayGames.OurUtils.Logger.d("Max match data size: " + mMaxMatchDataSize);
        }
        
        // called on game thread
        public void CreateQuickMatch(int minOpponents, int maxOpponents, int variant, 
                    Action<bool, TurnBasedMatch> callback) {
            GooglePlayGames.OurUtils.Logger.d(string.Format("AndroidTbmpClient.CreateQuickMatch, opponents {0}-{1}, var {2}",
            ...
            mClient.CallClientApi("tbmp create quick game", () => {
                ...
                if (!success) {
                    GooglePlayGames.OurUtils.Logger.w("Failed to create tbmp quick match: client disconnected.");
                    if (callback != null) {
        ...
        public void CreateWithInvitationScreen(int minOpponents, int maxOpponents, 
            ...
            GooglePlayGames.OurUtils.Logger.d(string.Format("AndroidTbmpClient.CreateWithInvitationScreen, " + 
            ...
            mClient.CallClientApi("tbmp launch invitation screen", () => {
                ...
                if (!success) {
                    GooglePlayGames.OurUtils.Logger.w("Failed to create tbmp w/ invite screen: client disconnected.");
                    if (callback != null) {
                        callback.Invoke(false, null);
                    }
                }
        ...
        public void AcceptFromInbox(Action<bool, TurnBasedMatch> callback) {
            GooglePlayGames.OurUtils.Logger.d(string.Format("AndroidTbmpClient.AcceptFromInbox"));
            ...
            mClient.CallClientApi("tbmp launch inbox", () => {
                ...
                if (!success) {
                    GooglePlayGames.OurUtils.Logger.w("Failed to accept tbmp w/ inbox: client disconnected.");
                    if (callback != null) {
                        callback.Invoke(false, null);
                    }
                }
        ...
        public void AcceptInvitation(string invitationId, Action<bool, TurnBasedMatch> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.AcceptInvitation invitationId=" + invitationId);
            TbmpApiCall("accept invitation", "acceptInvitation", null, callback, invitationId);
        }
        
        // called on game thread
        public void DeclineInvitation(string invitationId) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.DeclineInvitation, invitationId=" + invitationId);
        ...
        private void TbmpApiCall(string simpleDesc, string methodName,
            ...
            mClient.CallClientApi(simpleDesc, () => {
                ...
                if (!success) {
                    GooglePlayGames.OurUtils.Logger.w("Failed to " + simpleDesc + ": client disconnected.");
                    if (callback != null) {
        ...
        public void TakeTurn(string matchId, byte[] data, string pendingParticipantId, 
            ...
            GooglePlayGames.OurUtils.Logger.d(string.Format("AndroidTbmpClient.TakeTurn matchId={0}, data={1}, " + 
                    "pending={2}", matchId,
        ...
        public void Finish(string matchId, byte[] data, MatchOutcome outcome, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d(string.Format("AndroidTbmpClient.Finish matchId={0}, data={1} outcome={2}",
                    matchId, data == null ? "(null)" : data.Length + " bytes", outcome));
                        
            GooglePlayGames.OurUtils.Logger.d("Preparing list of participant results as Android ArrayList.");
            ...
            if (outcome != null) {
                foreach (string pid in outcome.ParticipantIds) {
                    GooglePlayGames.OurUtils.Logger.d("Converting participant result to Android object: " + pid);
                    ...
                    GooglePlayGames.OurUtils.Logger.d("Adding participant result to Android ArrayList.");
        ...
        public void AcknowledgeFinished(string matchId, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.AcknowledgeFinished, matchId=" + matchId);            
            TbmpApiCall("tbmp ack finish", "finishMatch", callback, null, matchId);
        }

        public void Leave(string matchId, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.Leave, matchId=" + matchId);
        ...
        public void LeaveDuringTurn(string matchId, string pendingParticipantId, 
                    Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.LeaveDuringTurn, matchId=" + matchId + ", pending=" + 
                    pendingParticipantId);
        ...
        public void Cancel(string matchId, Action<bool> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.Cancel, matchId=" + matchId);
        ...
        public void Rematch(string matchId, Action<bool, TurnBasedMatch> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.Rematch, matchId=" + matchId);
            TbmpApiCall("tbmp rematch", "rematch", null, callback, matchId);
        }
        
        public void RegisterMatchDelegate(MatchDelegate deleg) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.RegisterMatchDelegate");
            if (deleg == null) {
                GooglePlayGames.OurUtils.Logger.w("Can't register a null match delegate.");
                return;
            }
            ...
            if (mMatchFromNotification != null) {
                GooglePlayGames.OurUtils.Logger.d("Delivering pending match to the newly registered delegate.");
        ...
        private void OnSelectOpponentsResult(bool success, AndroidJavaObject opponents,
            ...
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.OnSelectOpponentsResult, success=" + success +
                     ", hasAutoMatch=" + hasAutoMatch);

            if (!success) {
                GooglePlayGames.OurUtils.Logger.w("Tbmp select opponents dialog terminated with failure.");
                if (callback != null) {
                    GooglePlayGames.OurUtils.Logger.d("Reporting select-opponents dialog failure to callback.");
                    PlayGamesHelperObject.RunOnGameThread(() => {
                        callback.Invoke(false, null);
                    });
                }
            ...
            GooglePlayGames.OurUtils.Logger.d("Creating TBMP match from opponents received from dialog.");
            ...
            mClient.CallClientApi("create match w/ opponents from dialog", () => {
                ...
                if (!ok) {
                    GooglePlayGames.OurUtils.Logger.w("Failed to create match w/ opponents from dialog: client disconnected.");
                    if (callback != null) {
                        callback.Invoke(false, null);
                    }
                }
        ...
        private void OnInvitationInboxResult(bool success, string invitationId, 
                Action<bool, TurnBasedMatch> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.OnInvitationInboxResult, success=" + success + ", " + 
                    "invitationId=" + invitationId);
            
            if (!success) {
                GooglePlayGames.OurUtils.Logger.w("Tbmp invitation inbox returned failure result.");
                if (callback != null) {
                    GooglePlayGames.OurUtils.Logger.d("Reporting tbmp invitation inbox failure to callback.");
                    PlayGamesHelperObject.RunOnGameThread(() => {
                        callback.Invoke(false, null);
                    });
                }
            ...
            GooglePlayGames.OurUtils.Logger.d("Accepting invite received from inbox: " + invitationId);
        ...
        private void OnInvitationInboxTurnBasedMatch(AndroidJavaObject matchObj,
                Action<bool, TurnBasedMatch> callback) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.OnInvitationTurnBasedMatch");
            
            GooglePlayGames.OurUtils.Logger.d("Converting received match to our format...");
            TurnBasedMatch match = JavaUtil.ConvertMatch(mClient.PlayerId, matchObj);
            GooglePlayGames.OurUtils.Logger.d("Resulting match: " + match);
            
            if (callback != null) {
                GooglePlayGames.OurUtils.Logger.d("Invoking match callback w/ success.");
                PlayGamesHelperObject.RunOnGameThread(() => {
        ...
        internal void HandleMatchFromNotification(TurnBasedMatch match) {
            GooglePlayGames.OurUtils.Logger.d("AndroidTbmpClient.HandleMatchFromNotification");
            GooglePlayGames.OurUtils.Logger.d("Got match from notification: " + match);
            
            if (mMatchDelegate != null) {
                GooglePlayGames.OurUtils.Logger.d("Delivering match directly to match delegate.");
                MatchDelegate del = mMatchDelegate;
                PlayGamesHelperObject.RunOnGameThread(() => {
                    del.Invoke(match, true);
                });
            } else {
                GooglePlayGames.OurUtils.Logger.d("Since we have no match delegate, holding on to the match until we have one.");
        ...
        private class ResultProxy : AndroidJavaProxy {
            ...
            public void onResult(AndroidJavaObject result) {
                GooglePlayGames.OurUtils.Logger.d("ResultProxy got result for method: " + mMethod);
                int statusCode = JavaUtil.GetStatusCode(result);
                bool isSuccess = mSuccessCodes.Contains(statusCode);
                TurnBasedMatch match = null;
                
                if (isSuccess) {
                    GooglePlayGames.OurUtils.Logger.d("SUCCESS result from method " + mMethod + ": " + statusCode);
                    if (mMatchCallback != null) {
                        GooglePlayGames.OurUtils.Logger.d("Attempting to get match from result of " + mMethod);
                        AndroidJavaObject matchObj = JavaUtil.CallNullSafeObjectMethod(result, "getMatch");
                        if (matchObj != null) {
                            GooglePlayGames.OurUtils.Logger.d("Successfully got match from result of " + mMethod);
                            match = JavaUtil.ConvertMatch(mOwner.mClient.PlayerId, matchObj);
                            matchObj.Dispose();
                        } else {
                            GooglePlayGames.OurUtils.Logger.w("Got a NULL match from result of " + mMethod);
                        }
                    }
                } else {
                    GooglePlayGames.OurUtils.Logger.w("ERROR result from " + mMethod + ": " + statusCode);
                }
                
                if (mSuccessCallback != null) {
                    GooglePlayGames.OurUtils.Logger.d("Invoking success callback (success=" + isSuccess + ") for " + 
                ...
                if (mMatchCallback != null) {
                    GooglePlayGames.OurUtils.Logger.d("Invoking match callback for result of method " + mMethod + ": " + 
